...
import { Send, User, Loader2, MessageSquare, PanelRightClose, PanelRightOpen, Mail, Globe, ArrowLeft, Plus, X, ChevronDown, Wifi, Trash2, UsersRound } from "lucide-react";
import { format } from "date-fns";
import { motion, AnimatePresence } from "framer-motion";
import { ChatUsernameOverlay } from "@/components/ChatUsernameOverlay";
import { cn } from "@/lib/utils";

type SidebarTab = "global" | "dm" | "groups" | "online";
...
export function SidebarChat() {
  ...
  useEffect(() => {
    const handleOpenChat = (e: Event) => {
      ...
      setIsVisible(true);
      if (detail.kind === "dm" && detail.user) {
        setPendingDmUser(detail.user);
        setActiveTab("dm");
      } else if (detail.kind === "group" && detail.groupId) {
          ...
          <div className="flex items-center justify-between">
            <div className="flex bg-background/50 border border-white/10 rounded-lg p-0.5 gap-0.5">
              ...
              <button
                ...
                data-testid="button-sidebar-tab-groups"
                className={cn(
                  "flex items-center gap-1.5 px-2.5 py-1.5 rounded-md text-[9px] font-bold uppercase tracking-wider transition-all duration-200",
                  activeTab === "groups"
                    ? "bg-secondary/15 text-secondary border border-secondary/30"
