...
export function Layout({ children, noContainer = false }: { children: React.ReactNode, noContainer?: boolean }) {
  ...
  return (
    <div className="min-h-screen bg-[radial-gradient(ellipse_at_top,_var(--tw-gradient-stops))] from-slate-900 via-background to-black flex font-body">
      ...
      {/* Neon edges */}
      <div className="fixed top-0 left-0 right-0 z-0 pointer-events-none" style={{ height:"1px", background:"linear-gradient(90deg, transparent, rgba(0,255,249,0.8) 30%, rgba(255,0,193,0.8) 70%, transparent)", animation:"edge-glow-h 4s ease-in-out infinite" }} />
      <div className="fixed bottom-0 left-0 right-0 z-0 pointer-events-none" style={{ height:"1px", background:"linear-gradient(90deg, transparent, rgba(255,0,193,0.8) 30%, rgba(0,255,249,0.8) 70%, transparent)", animation:"edge-glow-h 5s ease-in-out infinite 2s" }} />
      <div className="fixed top-0 left-0 bottom-0 z-0 pointer-events-none" style={{ width:"1px", background:"linear-gradient(180deg, transparent, rgba(139,0,255,0.8) 50%, transparent)", animation:"edge-glow-v 6s ease-in-out infinite 1s" }} />
      <div className="fixed top-0 right-0 bottom-0 z-0 pointer-events-none" style={{ width:"1px", background:"linear-gradient(180deg, transparent, rgba(0,255,249,0.8) 50%, transparent)", animation:"edge-glow-v 7s ease-in-out infinite 3s" }} />
